...
int main() {
    ...
    do {
        ...
        while (!game_over) {
            ...
            printf("\n\n");           //prints new lines
            if (aliens_alive == 0) {  //checks if aliens alive are 0, then the player wins the level
                printf("    YOU WIN! Level %d completed. Proceeding to level %d.\n", g_level, g_level + 1);
                g_level++;            //increases level after winning
                aliens_alive = 15;    //resets aliens for next level
                for (int r = 0; r < 3; r++) {
                    for (int c = 0; c < 5; c++) {
                        aliens[r][c].is_alive = 1;  //revives all the aliens
                        aliens[r][c].y = r + 1;     //resets y
                    }
                }
                player.is_alive = 1;  //revives player
                // score persists across levels
            } else {                  //if the player died, game over
                game_over = 1;
                ...
                if (score > g_high_score.score) {       //checks new high score
...
void draw_board() {
    ...
    printf("Press: [a] Left | [d] Right | [s] Shoot | [w] Remain\n");  //controls reminder
}
...
void display_instructions() {
    ...
    printf("Objective: Defend Earth by shooting down all the aliens (V) before they reach you (A).\n");
    printf("Controls:\n");
    printf("  - [a] Move left\n");
    printf("  - [d] Move right\n");
    ...
    scanf("%*c");  //waits for enter to proceed
}
